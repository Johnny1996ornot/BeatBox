// The DOMContentLoaded wrapper is the master lock that prevents the UI from crashing on load.
document.addEventListener('DOMContentLoaded', () => {
    ...
    function scheduler() {
        while (nextNoteTime < audioCtx.currentTime + 0.1) {
            ...
            if (currentStep >= STEPS) {
                ...
                if(!isRecording) {
                    stopAllVocals(); // Stops previous loop from echoing
    ...
    function stopAllVocals() { 
        audioClips.forEach(c => { 
            if(c.sourceNode) { try{ c.sourceNode.stop(); } catch(e){} c.sourceNode = null; }
        }); 
    }
    ...
    // Attach to window so HTML inputs can trigger it
    ...
    window.toggleRecord = async (trackId) => {
        ...
        try {
            ...
            setTimeout(() => { mediaRec.start(); isRecording = true; activeRecTrack = trackId; renderVocals(); }, 100);
    ...
    // Initial render call inside the DOMContentLoaded lock
    renderDrums(); 
    renderVocals();
});
